...
def create_app(test_config=None):
    ...
    @bp_query.route('upload', methods=['GET', 'POST'])
    def upload():
    ...
    @bp_query.route('uploaded', methods=['GET', 'POST'])
    def uploaded():
        if request.method == 'POST':
            removeFiles(app)
    ...
    @bp_query.route('filter', methods=['GET', 'POST'])
    def filter():
        if request.method == 'POST':
            filters = {'strain': [], 'type': [], 'Ecoli_sources': [], 'PAO1_sources': [], 'PA14_sources': [],
                       'verification': []}
            sources_Ecoli = ['EcoCyc', 'RegulonDB(Ecoli)', 'IMEx(Ecoli)', 'BindingDB(Ecoli)',
                             'EBI-GOA-nonIntAct(Ecoli)',
            ...
            for interaction in interactions:
                ...
                file_writer.writerow({psimi_fields[0]: interactor_ids[0], psimi_fields[1]: interactor_ids[1],
                                      ...
                                      psimi_fields[8]: refs['pmid'], psimi_fields[9]: taxid_A,
                                      psimi_fields[10]: taxid_B,
            ...
            return render_template('query/download.html')
        return render_template('query/filter.html')

    @bp_query.route('download')
    def download():
        try:
