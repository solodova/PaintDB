import csv
from Schema1 import Interactor, OrthologEcoli, Interaction, InteractionReference, InteractionSource, Metabolite
from Parsers.Parser import is_experimental_psimi

def parse_ecoli_mint(session):
    with open('Ecoli/PSICQUIC/MINT.txt') as csvfile:
        ...
        for row in reader:
            ...
            for interactor_pair in interactors:
                ...
                source = session.query(InteractionSource).filter(InteractionSource.interaction_id == interaction.id,
                                                                 InteractionSource.data_source == 'MINT').first()

                if source is None:
                    source = InteractionSource(interaction_id=interaction.id, data_source='MINT')
                    session.add(source)
        session.commit()
